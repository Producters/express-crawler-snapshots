...
Pool.prototype.spawnInstance = function spawnInstance(cb) {
    ...
    self.log('log', 'starting phantomjs instance ' + instance_no + '...');
    phantom.create("--web-security=no", "--ignore-ssl-errors=yes", 
        {
            onExit: function () {
                self.log('log', 'phantomjs instance ' + instance_no + ' terminated'); 
                if (instance) {
                    instance.onExit();
                } else {
            ...
            if (error) {
                ...
                self.log('log', 'phantomjs instance ' + instance_no + ' started');
...
Pool.prototype.processQueue = function () {
    if (this.queued_get_instance_callbacks.length) {
        this.log('log', 'processing next queued request, queue length is now ' + (this.queued_get_instance_callbacks.length -1 ))
...
Pool.prototype.getInstance = function getInstance(cb) {
    if (this.available_instances.length) {
        ...
        this.log('log', 'queued a request for phantomjs instance. queue length is ' + this.queued_get_instance_callbacks.length);
    }
};